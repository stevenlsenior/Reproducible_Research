...
There are two variables in the dataset that relate to health impacts INJURIES and FATALITIES. Before doing the full data analysis, it is useful to do have a basic look at the distribution of the data, calculate some simple summary statistics, and check for missing data.
...
```{r}
# Calculate mean deaths for each event type
avg_injury <- storm_health %>%
				   summarise(mean_injury = mean(INJURIES)) %>% 
				   arrange(desc(mean_injury))

head(avg_injury, n = 10)
```
...
There is no specific guidance for this project on how to define harm to population health. In the absence of guidance on how to weight deaths versus injuries, I will use a combined measure for each event type that is simply the total number of deaths and injuries caused. 

```{r}
# Create TOTAL_HARM variable
storm_health$TOTAL_HARM <- storm_health$INJURIES + storm_health$FATALITIES

# Calculate sum and mean of TOTAL_HARM for each event type
avg_harm <- storm_health %>%
				 summarise(mean_harm = mean(TOTAL_HARM)) %>% 
				 arrange(desc(mean_harm))

head(avg_harm, n = 10)

total_harm <- storm_health %>%
				   summarise(total_harm = sum(TOTAL_HARM)) %>% 
				   arrange(desc(total_harm))

head(total_harm, n = 10)
```

From this we can see that wild fires cause the most harm on average, and tornados cause the most harm in total. Finally, I construct plots of total and average harm for the top ten events in each category.

```{r, fig.width=12}
par(mfrow = c(1,2), mar = c(10, 5, 4, 1), las = 2)

barplot(total_harm$total_harm[1:10],
	  ylab = "Fatalities & Injuries",
	  names.arg = total_harm$EVTYPE[1:10],
	  cex.names = 0.6,
	  cex.axis = 0.7,
	  main = "Total harm by event type"
	  )

barplot(avg_harm$mean_harm[1:10], 
	  ylab = "Fatalities & Injuries",
	  names.arg = avg_harm$EVTYPE[1:10], 
	  cex.names = 0.6,
	  cex.axis = 0.7,
	  main = "Mean harm by event type"
	  )
```
